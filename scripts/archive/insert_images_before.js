...
files.forEach(f => {
  ...
  content = content.replace(/(\s*)<section class="glass article-card" style="margin-bottom: 3rem;">/, (match, p1) => {
    return p1 + '<div style="text-align: center; margin-bottom: 3rem;">\n' +
           p1 + '    <img src="' + f.img + '" alt="事件のイメージ画像" style="max-width: 75%; height: auto; border-radius: 12px; box-shadow: 0 8px 32px rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1);">\n' +
           p1 + '</div>\n' +
           match;
  });
